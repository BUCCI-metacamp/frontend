...
const ProductChartCard = (props) => {
  ...
  useEffect(() => {
    if (data && typeof data.totalFailCount === 'number') {
      const formattedData = {
  ...
  if (!data || typeof data.totalFailCount !== 'number' || chartData.length === 0) {
          ...
          <Line
            ...
            stroke={
              ...
              "var(--color-totalFailCount)" : 
                dataKey == 'failCount' ? "var(--color-failCount)" : "var(--color-totalPassCount)"}
            strokeWidth={2}
            dot={false}
          />
        </LineChart>
