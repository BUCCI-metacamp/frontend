...
const Unit2Chart = ({ data }) => {
  ...
  const [currentTime, setCurrentTime] = useState(new Date());
    ...
    useEffect(() => {
      if (data && data.length > 0) {
        const filtered = data.filter(item => item.name === 'No2Standby');
  ...
  return (
    <div>
    <Card className="bg-[#262852] border-0">
      <CardHeader className="pb-2">
        <div className='flex flex-row justify-between items-center'>
        <CardTitle className="text-2xl text-white">가공 공정</CardTitle>
        ...
        <CardDescription>가공 공정 푸셔 상태</CardDescription>
      </CardHeader>
      <CardContent>
          <div className="flex justify-between">
            ...
            <div className="flex flex-col">
            <div className='flex justify-center'>
            {data.filter(item => item.name === 'No2InPoint')[0].value ? (
                <div className='bg-red-500 w-[16px] h-[16px] rounded-sm'/>
              ) : (
                <div className='bg-slate-300 w-[16px] h-[16px] rounded-sm'/>
              )}
            ...
            <div className="flex flex-col">
            <div className='flex justify-center'>
            { data.filter(item => item.name === 'No2OutPoint')[0].value ? (
                <div className='bg-red-500 w-[16px] h-[16px] rounded-sm'/>
              ) : (
                <div className='bg-slate-300 w-[16px] h-[16px] rounded-sm'/>
              )}
